...
function Navbar({ setView, activeView, tags = [] }) {
  ...
  return (
    <div className="navbar-container">
      ...
      <div className="tags-section">
        ...
        <div className="tags-list">
          {tags.map((tag) => (
            <button
              key={tag.id || tag}
              className={`nav-item-desktop ${activeView === `tag-${tag.id}` ? "active" : ""}`}
      ...
      <button
        className={`nav-item-desktop settings-nav-link ${activeView === "settings" ? "active" : ""}`}
        onClick={() => setView("settings")}
      >
        <div className="nav-item-left">
          <img src="/assets/images/icon-settings.svg" alt="Settings icon" />
