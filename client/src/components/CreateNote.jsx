...
function CreateNote({ setView, onCreated }) {
  ...
  const handleSave = async () => {
    if (!title.trim()) {
      addToast("Add a title before saving.", { type: "error" });
      return;
    }
    const tagsArray = tagInput.split(",").map(t => t.trim()).filter(t => t !== "");
    try {
      const { data: note } = await apiClient.post("/notes", {
        title,
        content: editor.getHTML(),
        tagNames: tagsArray
      });
      addToast("Note created successfully!");
      if (onCreated) await onCreated(note);
      else if (setView) setView("all");
    } catch (err) {
